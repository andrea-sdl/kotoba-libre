...
The System tab also controls MCP auto-reconnect. It is enabled by default and checks every 30 minutes, plus every time the main LibreChat window is shown or finishes loading. First-open checks also get one short settled retry so the reconnect pass is not lost before LibreChat has mounted its app shell. The reconnect pass only runs on the configured LibreChat origin, uses the embedded page session to read `/api/mcp/connection/status`, refreshes LibreChat's bearer token if the API requires it, and skips connected or connecting servers. For disconnected or erroring servers, it clicks LibreChat's own visible MCP `Connect` or `Reconnect` controls one at a time when the app UI is mounted, checks status after each click, and posts to each server's reinitialize endpoint only when the UI path did not move it to connected or connecting. If LibreChat returns OAuth URLs, the page bridge opens them in order and polls LibreChat's connection status until each reconnect finishes or times out. When a reconnect succeeds while no response is being generated, the page refreshes so LibreChat reloads its MCP state.
